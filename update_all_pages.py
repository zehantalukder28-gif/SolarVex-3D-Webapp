...
<body>
      ...
      <div class="menu-nav">
        ...
        <div class="menu-item-group">
          ...
          <div class="menu-sub-dropdown">
            <a href="/solution-coastal-villa" class="menu-sub-item hover-orange">— Coastal Villa Solution</a>
            <a href="/solution-urban-grid" class="menu-sub-item hover-orange">— Urban Grid Solution</a>
          </div>
        </div>
        <div class="menu-item-group">
          <a href="products.html" class="menu-item hover-cyan">Products</a>
          <div class="menu-sub-dropdown">
            <a href="/product-modules" class="menu-sub-item hover-cyan">— Monocrystalline PV Modules</a>
            <a href="/product-inverters" class="menu-sub-item hover-cyan">— Smart Wave Inverters</a>
            <a href="/product-storage" class="menu-sub-item hover-cyan">— High-Capacity Storage Cells</a>
          </div>
        </div>
        <div class="menu-item-group">
          <a href="cases.html" class="menu-item hover-cyan">Cases</a>
          <div class="menu-sub-dropdown">
            <a href="case-coastal.html" class="menu-sub-item hover-orange">— Coastal Wind Resistance Array</a>
            <a href="case-urban.html" class="menu-sub-item hover-orange">— High-Density Urban Rooftop</a>
          </div>
        </div>
  ...
  <script>
        ...
        if (window.location.hash) {
            setTimeout(() => {
                if (typeof lenis !== 'undefined') {
                    lenis.scrollTo(window.location.hash, { offset: -100, immediate: true });
                } else {
    ...
    /* ========================================
       PREMIUM CROSS-FADE PAGE TRANSITIONS
       ======================================== */
    const transitionCurtain = document.getElementById('pageTransitionCurtain');
    
    // Inbound: fade curtain out to reveal page content
    gsap.fromTo(transitionCurtain,
      { opacity: 1 },
      { opacity: 0, duration: 0.5, ease: 'power2.out', delay: 0.05, onComplete: () => {
        transitionCurtain.style.pointerEvents = 'none';
        transitionCurtain.style.display = 'none';
      }}
    );
    ...
    window.addEventListener('pageshow', function(event) {
        transitionCurtain.style.pointerEvents = 'none';
        transitionCurtain.style.display = 'none';
        gsap.set(transitionCurtain, { opacity: 0 });
    ...
    // Intercept internal links for outbound cross-fade transition
    document.querySelectorAll('a').forEach(anchor => {
      anchor.addEventListener('click', function(e) {
        let target = this.getAttribute('href');
        if (!target) return;
        
        // Clean URL routing fallback mapping
        const routeMap = {
          '/product-modules': 'product-modules.html',
          '/product-inverters': 'product-inverter.html',
          '/product-storage': 'product-storage.html',
          '/solution-urban-grid': 'project-industrial.html',
          '/solution-coastal-villa': 'solution-coastal.html'
        };
        if (routeMap[target]) {
          target = routeMap[target];
        }
        
        const isTransitionTarget = target.includes('.html') || Object.values(routeMap).includes(target);
        if(isTransitionTarget && !this.hasAttribute('target') && !this.classList.contains('product-card') && this.id !== 'modalActionBtn') {
          const isSamePageHash = target.includes('#') && (
            (target.startsWith('index.html') && (window.location.pathname === '/' || window.location.pathname.endsWith('index.html'))) ||
            (window.location.pathname.endsWith(target.split('#')[0]))
          );
          ...
          // OUTBOUND: Cross-fade content out, then navigate
          e.preventDefault();
          const fullscreenMenu = document.getElementById('fullscreenMenu');
          if (fullscreenMenu) fullscreenMenu.classList.remove('open');
